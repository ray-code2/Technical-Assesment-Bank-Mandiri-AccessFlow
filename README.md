...
## Suggested assessment demo video

1. Start with `docker compose up --build` and show all three services healthy.
2. Sign in as Alice, submit a request, and show it as “Waiting for manager.”
3. Sign in as Bob, approve it, and emphasize that it remains `IN_PROGRESS` but moves to admin.
4. Sign in as Carol, show the system dashboard and give final approval.
5. Return to Alice and show the request as `APPROVED`.
6. Demonstrate a rejection and the admin-only catalog.
7. Briefly show the workflow tests and the transaction/authorization code.

For a detailed implementation explanation, read [`docs/IMPLEMENTATION_GUIDE.md`](docs/IMPLEMENTATION_GUIDE.md). For a presentation-ready technical and non-technical briefing, use [`docs/WHAT_TO_KNOW.md`](docs/WHAT_TO_KNOW.md).
